...
export function initMotion(): void {
  ...
  /* ──────────────────────────────────────────────────────────
     Estado activo del dock (barra de navegación inferior)
     El dock usa transition:persist, así que no se re-monta entre
     navegaciones: hay que reajustar el ítem activo según el pathname
     en cada astro:page-load.
     ────────────────────────────────────────────────────────── */
  const here = (location.pathname.replace(/\/+$/, '') || '/');
  document.querySelectorAll<HTMLAnchorElement>('.dock [data-path]').forEach((a) => {
